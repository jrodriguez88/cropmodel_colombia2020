# Script Practice 2 - Parametrizacion GDD de AquaCrop
...
get_param_gdd <- function(data , cropfile, sowing_dates) {
  ...
  sow_dates <- gdd_data %>% dplyr::filter(month(date) %in% sowing_month)
...
get_param_gdd(data_clima, crop_file, sowing_month)
